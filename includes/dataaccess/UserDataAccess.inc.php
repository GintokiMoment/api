...
class UserDataAccess extends DataAccess{
	...
	*/
	function convertModelToRow($user){
		$row = [];
		$row["user_id"] = intval(mysqli_real_escape_string($this->link, $user->id));
		...
		// Move this line after setting "user_password" key
		$row["user_password"] = $this->saltAndHashPassword($row["user_salt"], $user->password);
		$row["user_active"] = boolval($user->active);
	...
	*/
	function convertRowToModel($row){
		...
		$user->id = intval(mysqli_real_escape_string($this->link, $row["user_id"])); 
		$user->firstName = $this->sanitizeHTML($row["user_first_name"]);
		$user->lastName = $this->sanitizeHTML($row["user_last_name"]);
		$user->email = $this->sanitizeHTML($row["user_email"]);
		$user->roleId = $row["user_role"];
		$user->password = "";
		$user->salt = ""; // never send salt
		$user->active = boolval($row["user_active"]);


		return $user; // comment this out when you implement the code for this method
	}
	...
	*/
	function getAll($args = null){
		...
		while($row = mysqli_fetch_assoc($result)){
			$allUsers[] = $this->convertRowToModel($row);
		}
